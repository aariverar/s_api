...
def delete_log(table_client,partition_key,row_key):
    try:
        table_client.delete_entity(partition_key=partition_key,row_key=row_key)
        print(f"El registro {partition_key},{row_key} se ha eliminado")
    except Exception as e:
